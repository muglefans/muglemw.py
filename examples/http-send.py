...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='send mugle use http/https')
    parser.add_argument('amount', metavar='1', type=str, nargs=1,
                    help='the amount of mugle to send')
    parser.add_argument('-u', '--url', dest='url', type=str,
                    help='the http/https url to where mugle will be sent ')

    args = parser.parse_args()
    amount = int(Decimal(args.amount[0]) * 1000000)

    home = str(Path.home())
    api_url = 'http://localhost:6820/v3/owner'
    api_sercet_file = os.path.join(home, '.mugle/main/.owner_api_secret')
    api_user = 'mugle'
